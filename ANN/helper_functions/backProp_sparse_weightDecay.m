function [updated_weight_layers, updated_bias_layers, last_weight_change_layers, last_bias_change_layers] = backProp(weight_layers, activation_layers, bias_layers, input, y, y_hat, eta_o, eta_h, last_weight_change_layers, last_bias_change_layers, beta, roh, roh_hat_matrix)
...
%   
    alpha = 0.8; %for momentum (set to 0 if you want no momentum)
    ...
    for i = end_of_network:-1:1 %starting form the back...
...
%                 delta_layers{i} = activationDerivative(activation_layers{i}) * sum(weight_layers{i+1}' * delta_layers{i+1});
                delta_layers{i} = activationDerivative(activation_layers{i}) * sum((weight_layers{i+1}' * delta_layers{i+1}) - beta .* ((1 - roh) ./ (1 - roh_hat_matrix{i}) - (roh ./ roh_hat_matrix{i})));
            end
    end
